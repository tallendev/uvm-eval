...
def count_occurrences_all(numbers):
    ...
    keys = d.keys()
    return keys, [d[val] for val in keys]

def process_batch(i, b, fmin, ranges, args):
    faults = sorted([f.fault_address - fmin for f in b])
    unique, counts = count_occurrences(faults)
    outdir = "batchdups"
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    plot_batch(unique, counts, i, ranges, args, outdir)

def process_batch_all(i, b, fmin, ranges, args):
...
def plot_batch(x, y, batchnum, ranges, args, outdir):
...
def main():
    ...
    with Pool(processes=NUM_THREADS) as pool:
        pool.starmap(process_batch, [(i, batches[i], fmin, ranges, args) for i in iters])
