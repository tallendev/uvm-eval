...
void cg_init(
        ...
        MKL_INT* a_row_index, MKL_INT* a_col_index, 
        double* a_non_zeros);

void cg_calc_w(
        const int x, const int y, const int halo_depth, double* pw,
        double* p, double* w, double* kx, double* ky, 
        MKL_INT* a_row_index, MKL_INT* a_col_index, double* a_non_zeros);
...
void cheby_init(const int x, const int y,
        ...
        double* ky, MKL_INT* a_row_index, MKL_INT* a_col_index,
        double* a_non_zeros);
...
void cheby_iterate(const int x, const int y,
        ...
        double* kx, double* ky, MKL_INT* a_row_index,
        MKL_INT* a_col_index, double* a_non_zeros); 

// Jacobi solver kernels
void jacobi_init(
        const int x, const int y, const int halo_depth, const int coefficient, 
        double rx, double ry, double* density, double* energy, double* u0, 
        double* u, double* kx, double* ky);
void jacobi_iterate(
        const int x, const int y, const int halo_depth, double* error, 
        double* kx, double* ky, double* u0, double* u, 
        double* r);
