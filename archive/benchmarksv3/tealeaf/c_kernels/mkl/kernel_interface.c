...
{
    set_chunk_state(
            chunk->x, chunk->y, chunk->vertex_x, chunk->vertex_y, chunk->cell_x, 
            chunk->cell_y, chunk->density, chunk->energy0, chunk->u, 
        ...
        Chunk* chunk, Settings* settings)
{
    kernel_finalise(
            ...
            chunk->cheby_alphas, chunk->cheby_betas, chunk->ext->a_row_index,
        ...
        double rx, double ry, double* rro)
{
    START_PROFILING(settings->kernel_profile);
    cg_init(
            chunk->x, chunk->y, settings->halo_depth, settings->coefficient, 
            rx, ry, rro, chunk->density, chunk->energy, chunk->u, 
            chunk->p, chunk->r, chunk->w, chunk->kx, 
            chunk->ky, chunk->ext->a_row_index, chunk->ext->a_col_index, 
            chunk->ext->a_non_zeros);
    STOP_PROFILING(settings->kernel_profile, __func__);
}
        ...
        Chunk* chunk, Settings* settings, double alpha, double beta)
{
    ...
    ppcg_inner_iteration(
            chunk->x, chunk->y, settings->halo_depth, alpha, beta, 
            chunk->u, chunk->r, chunk->kx, chunk->ky,
            chunk->sd);
    STOP_PROFILING(settings->kernel_profile, __func__);
}
